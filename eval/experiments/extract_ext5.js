/**
 * Iteration 5 — extract the 21 features for source5.csv rows (dmitva) using the
 * exact approach of extract_ext.js (index.html script eval + 4 readability
 * features, identical feature order), then write features_ext5.csv =
 * features_ext.csv rows + new source5 feature rows.
 *
 * Usage: node eval/experiments/extract_ext5.js
 */

const fs = require('fs');
const path = require('path');

// ─── Minimal DOM / browser stubs (copied from extract_ext.js) ────────────────
const _el = () => ({
...
// ─── Feature names — order MUST match extract_ext.js ────────────────────────
const FEATURE_NAMES = [
...
// ─── 4 readability candidates (copied verbatim from extract_ext.js) ──────────
...
// ─── CSV parsing (copied from extract_ext.js) ────────────────────────────────
...
// ─── Main ────────────────────────────────────────────────────────────────────
const csvPath = path.join(__dirname, 'source5.csv');
...
// features_ext5.csv = features_ext.csv rows + new source5 rows
const basePath = path.join(__dirname, 'features_ext.csv');
const base = fs.readFileSync(basePath, 'utf8').replace(/^﻿/, '').trimEnd();
const baseHeader = base.split('\n')[0];
const expectedHeader = [...FEATURE_NAMES, ...EXT_NAMES, 'is_ai_generated', 'source'].join(',');
if (baseHeader !== expectedHeader) {
  process.stderr.write(`ERROR: features_ext.csv header mismatch\n  got: ${baseHeader}\n  exp: ${expectedHeader}\n`);
  process.exit(1);
}
const outPath = path.join(__dirname, 'features_ext5.csv');
fs.writeFileSync(outPath, base + '\n' + newRows.join('\n'), 'utf8');
console.log(`\nAppended ${kept} source5 rows (skipped ${skipped}) -> ${outPath} (${base.split('\n').length - 1} base rows + ${kept})`);
